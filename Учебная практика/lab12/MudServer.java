...
import lab12.Mud.RemoteMudPerson;
...
public class MudServer extends UnicastRemoteObject 
    implements Mud.RemoteMudServer, Serializable
{
    ...
    Hashtable<String,Mud.RemoteMudPlace> places;   // A mapping of place names to places in this MUD
	...
	throws RemoteException
    {
        ...
        this.places = new Hashtable<String,Mud.RemoteMudPlace>();
        // Create the entrance place
        try { this.entrance = new MudPlace(this, placename, description); } 
        catch (Mud.PlaceAlreadyExists e) {} // Should never happen
    }
    ...
    public Mud.RemoteMudPlace getEntrance() throws RemoteException { 
        return entrance; 
    }
     ...
     **/
    public Mud.RemoteMudPlace getNamedPlace(String name) 
              throws RemoteException, Mud.NoSuchPlace
    {
        Mud.RemoteMudPlace p = (Mud.RemoteMudPlace) places.get(name);
        if (p == null) throw new Mud.NoSuchPlace();
        return p;
    }
     ...
     **/
    public void setPlaceName(Mud.RemoteMudPlace place, String name) 
	throws Mud.PlaceAlreadyExists
    {
        if (places.containsKey(name)) throw new Mud.PlaceAlreadyExists();
	...
	throws RemoteException, Mud.BadPassword, IOException
    {
        if ((this.password != null)&& !this.password.equals(password)) 
            throw new Mud.BadPassword();
    ...
    public static void main(String[] args) {
        try {
            ...
            while(true){
                Thread.sleep(300000);
                server.places.forEach((s, p) ->{
                    RemoteMudPerson person = null;
                    try {
                        for (var name : p.getNames()){
                            person = p.getPerson((String)name);
                            person.getDescription();
                        }
                    } catch (Mud.NoSuchPerson ex) {
