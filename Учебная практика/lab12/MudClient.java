...
public class MudClient {
    ...
    public static void main(String[] args) {
        try {
            ...
            // in the registry.
            Mud.RemoteMudServer server = 
                (Mud.RemoteMudServer)Naming.lookup("rmi://" + hostname + "/" +
            ...
            Mud.RemoteMudPlace location = null;
            if (placename == null) location = server.getEntrance();
            else location = (Mud.RemoteMudPlace) server.getNamedPlace(placename);
     ...
     **/
    public static void runMud(Mud.RemoteMudPlace entrance, MudPerson me) 
	throws RemoteException
    {
        Mud.RemoteMudPlace location = entrance;  // The current place
        ...
        for(;;) {  // Loop until the user types "quit"
            try {    // Catch any exceptions that occur in the loop
                ...
                // DESCRIBE: Describe a named person
                    ...
                    try { 
                        Mud.RemoteMudPerson p = location.getPerson(arg);
            ...
            // Handle the many possible types of MudException
                ...
                if (e instanceof Mud.NoSuchThing) 
                    System.out.println("There isn't any such thing here."); 
                else if (e instanceof Mud.NoSuchPerson) 
                   System.out.println("There isn't anyone by that name here.");
                else if (e instanceof Mud.NoSuchExit) 
                  System.out.println("There isn't an exit in that direction.");
                else if (e instanceof Mud.NoSuchPlace) 
                    System.out.println("There isn't any such place."); 
                else if (e instanceof Mud.ExitAlreadyExists)
                    System.out.println("There is already an exit " +
				       "in that direction.");
                else if (e instanceof Mud.PlaceAlreadyExists)
                    System.out.println("There is already a place " +
				       "with that name.");
                else if (e instanceof Mud.LinkFailed)
                    System.out.println("That exit is not functioning.");
                else if (e instanceof Mud.BadPassword) 
                    System.out.println("Invalid password."); 
                else if (e instanceof Mud.NotThere)      // Shouldn't happen
                    System.out.println("You can't do that when " +
				       "you're not there."); 
                else if (e instanceof Mud.AlreadyThere)  // Shouldn't happen
     ...
     * the current place.
     **/
    public static void look(Mud.RemoteMudPlace p) 
	throws RemoteException, Mud.MudException
    {
